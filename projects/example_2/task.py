...
class Task(InferenceTask):
    def __init__(self):
        ...
        self.dataset = load_dataset(
            "nvidia/Nemotron-Instruction-Following-Chat-v1", streaming=False
        )["chat_if"]
        self._cur.execute(
            "CREATE TABLE IF NOT EXISTS result(id INT PRIMARY KEY,content TEXT);"
        )
        load_dotenv(path.join(dirname(__file__), ".env"))
        self._client = AsyncOpenAI(
            api_key=os.environ["API_KEY"], base_url=os.environ["BASE_URL"], timeout=None
        )
    ...
    async def process(self, data, order: int, sem: Semaphore, bar: tqdm.tqdm):
        ...
        if (
            self._cur.execute(
                "SELECT COUNT(*) FROM result WHERE id=?;", (order,)
            ).fetchone()[0]
            > 0
        ):
            bar.update(1)
            ...
            for message in input_json:
                ...
                while True:
                    try:
                        ...
                        "以下に外国語の文章が与えられるので、外国語の文章を全て日本語に翻訳してください。なお、以下の条件を**遵守**すること。\n"
                        "\n"
                        ...
                        if sleep_time > 16.0:
                            translated_messages.append(
                                {"role": "assistant", "content": "<-- output is missing -->"}
                            )
                            break
        ...
        self._cur.execute(
            "REPLACE INTO result(id, content) VALUES (?,?);",
